...
const story = {
    ...
    ending: {
        text: "The shelves begin to blur and dissolve into static. The test is complete. Your psychological profile has been calculated based on your path.",
        choices: [] // Handled dynamically in code
    }
};
...
function showScene(sceneKey) {
    ...
    if (sceneKey === "ending") {
        ...
        if (highestStat === "nostalgia") {
            resultText += "Result 1: You are stuck looking backward (Nostalgia/Melancholy). You find comfort in old memories even when they haunt you.";
        } else if (highestStat === "survival") {
            resultText += "Result 2: You are hyper-fixated on the immediate moment (Survival/Anxiety). You feel every shift in your environment intensely.";
        } else {
            resultText += "Result 3: You are solely focused on finding the exit (Impatience/Drive). You refuse to let liminal spaces slow you down.";
        }

        dialogueDiv.innerText = resultText;
        return;
    }

    dialogueDiv.innerText = currentScene.text;
